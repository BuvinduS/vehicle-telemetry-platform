...
def build_split(df: pd.DataFrame):
    ...
    return train_df, val_df, special_df
...
if __name__ == "__main__":
    ...
    train_df, val_df, special_df = build_split(df)
    ...
    print(f"\nCompression from resampling: "
          f"{len(df[df['condition'].isin(NORMAL_CONDITIONS)]):,} -> "
          f"{len(train_df) + len(val_df):,} rows "
          f"({(len(train_df) + len(val_df)) / len(df[df['condition'].isin(NORMAL_CONDITIONS)]) * 100:.1f}%)")
    ...
    # result as last time.
    manifest_rows = []
    for split_name, split_df in [("train", train_df), ("val", val_df), ("holdout_special", special_df)]:

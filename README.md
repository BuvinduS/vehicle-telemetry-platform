...
- Docker file created with PostgrSQL TimeScaleDB image in `analytics/`. To be moved to root level once functionality is confirmed.
...
- Implemented a jupyter notebook to 'explore' the data (`analytics/notebooks/01_explore.ipynb`)
    - The timescale database contains mock data. Therefore no logical implications of correlations can yet be obtained. But pipeline between the DB and the system is verified through this.
...
- Defaulted to using the compose.yaml at project root by deleting the previous container and recreating at the root. Compose file inside analytic is now redundant.
- Moved database schema to project root and add continuous aggregate view. Without this continuous aggregate view without this new rows will not appear until an aggregate view is created manually.

#### Summary for week 3: 
Data pipeline between the DB, the analytics subsystema and grafana is functional. Should work as intended once mock data is available since the same schema will be used system wide.
